keep.out.unsb <- function(model, wl1, wl2, wl3, wl4){
  ...
  if(!is.na(wl3) & wl1 != min(model$wl)) keepout.return <- paste0(keepout.1, "-", keepout.2, ",", keepout.3, "-", keepout.4, ",", keepout.5, "-", keepout.max)
  if(is.na(wl3) & wl1 != min(model$wl)) keepout.return <- paste0(keepout.1, "-", keepout.2, ",", keepout.5, "-", keepout.max)
  if(is.na(wl3) & wl1 == min(model$wl)) keepout.return <- paste0(keepout.1, "-", keepout.max)
